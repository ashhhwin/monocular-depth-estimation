...
Supports both NEW (5-feature) and OLD (3-branch) models.
- Model 1: ImprovedDistanceEstimator (EfficientNet-B2 + 5 features)
- Model 2: HybridGeometricEstimator (3-branch)

Fixes:
- Integrated new 5-feature model architecture for Model 1.
...
# ============================================================================
# MODEL 1: IMPROVED DISTANCE ESTIMATOR (NEW 5-FEATURE MODEL)
# ============================================================================
...
class ImprovedDistanceEstimator(nn.Module):
    """Model 1: Multi-Scale EfficientNet-B2 + 5-Feature Fusion"""
    def __init__(self, backbone="efficientnet_b2", pretrained=True):
        ...
        self.mid_attention = CBAM(self.mid_dim)
        self.high_attention = CBAM(self.high_dim)
        self.global_pool = nn.AdaptiveAvgPool2d(1)
        
        self.feature_fusion = nn.Sequential(
            nn.Linear(self.mid_dim + self.high_dim, 512),
            nn.ReLU(),
            nn.Dropout(0.3)
        )
        
        # USE NEW 5-FEATURE ENCODER
        self.depth_encoder = ReducedDepthEncoder(num_features=5)
    ...
    def forward(self, images, depth_scalars):
        ...
        mid_features = self.mid_attention(mid_features)
        high_features = self.high_attention(high_features)
...
def process_with_model1(image_data, detector, model1, depthpro_processor, depthpro_model, device):
    """Process with ImprovedDistanceEstimator (Model 1)"""
    start_time = time.time()
    
    result = {
        'model': 'ImprovedDistanceEstimator',
...
def main():
    ...
    st.markdown("**Compare Model 1 (ImprovedDistanceEstimator) vs Model 2 (Hybrid Geometric Estimator)**")
    ...
    run_model1 = st.sidebar.checkbox("Run Model 1: ImprovedDistanceEstimator (5-Feature)", value=True)
        ...
        with st.spinner("Loading models..."):
            try:
                ...
                if run_model1:
                    ...
                    # LOAD NEW MODEL
                    model1 = ImprovedDistanceEstimator(backbone="efficientnet_b2", pretrained=False)
    ...
    if len(st.session_state.results_model1) > 0 or len(st.session_state.results_model2) > 0:
        ...
        if result1 and result2:
            ...
            # LEFT: MODEL 1
            with col_left:
                st.markdown("### Model 1: ImprovedDistanceEstimator")
        ...
        # SINGLE MODEL VIEW
            ...
            st.subheader("Model 1: ImprovedDistanceEstimator Results")
        ...
        if len(valid_results1) > 0 or len(valid_results2) > 0:
            ...
            if len(valid_results1) > 0 and len(valid_results2) > 0:
                with cols[2]:
                    st.markdown("**Winner**")
                    if mae1 < mae2:
                        st.success("Model 1 (ImprovedDistanceEstimator)")
